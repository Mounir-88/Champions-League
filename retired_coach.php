...
<body>
    ...
    <div class="container">
        ...
        // SQL query to retrieve matches where the away team's coach has retired from playing
        $sql = "SELECT m.*, c.coach_name, c.retired_player
                FROM Matches m
                INNER JOIN Team t ON m.awayTeam_id = t.team_id
                INNER JOIN Coach c ON t.team_id = c.team_id
                WHERE c.retired_player = 'r'";
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
            // Output data of each row
            echo '<table class="matches-table">';
            echo '<tr><th>Match ID</th><th>Date</th><th>Home Team</th><th>Away Team</th><th>Home Team Score</th><th>Away Team Score</th><th>Winner</th><th>Spectators</th><th>Referee</th><th>Coach</th></tr>';
